...
exports.inject = exports.UNGROUPED_LABEL = void 0;
exports.matchDraftsHotkey = matchDraftsHotkey;
exports.workspaceBaseLabel = workspaceBaseLabel;
exports.selectDraftRows = selectDraftRows;
exports.draftAge = draftAge;
...
exports.DraftsFooterAction = DraftsFooterAction;
exports.apply = apply;
...
exports.UNGROUPED_LABEL = 'Ungrouped';
/** basename of a path ("both separators accepted"), or the fallback label. */
function workspaceBaseLabel(path) {
    if (path === undefined || path === '')
        return exports.UNGROUPED_LABEL;
 ...
 */
function selectDraftRows(sessions, workspaces) {
...
function draftAge(updatedAt, now) {
    const diff = Math.max(0, now - updatedAt);
    if (diff < 60_000)
        return { key: 'now', n: 0 };
    if (diff < 3_600_000)
        return { key: 'min', n: Math.floor(diff / 60_000) };
    if (diff < 86_400_000)
        return { key: 'hour', n: Math.floor(diff / 3_600_000) };
...
function DraftsFooterAction(props) {
    ...
    (0, react_1.useEffect)(() => {
        const onKey = (event) => {
            ...
            const action = matchDraftsHotkey(event);
            if (action === null)
                return;
    ...
    (0, react_1.useLayoutEffect)(() => {
        ...
        const place = () => {
            ...
            if (top < PANEL_MARGIN)
                top = Math.min(rect.bottom + PANEL_GAP, Math.max(PANEL_MARGIN, window.innerHeight - height - PANEL_MARGIN));
    ...
    const onOpen = (0, react_1.useCallback)((sessionId) => {
        setOpen(false);
        openSession(sessionId);
    }, [openSession]);
    const onDiscard = (0, react_1.useCallback)((sessionId) => {
    ...
    const rows = drafts.map((draft) => {
        ...
        return ((0, jsx_runtime_1.jsxs)("div", { className: `dsd-row${draft.current ? ' dsd-current' : ''}`, role: "button", tabIndex: 0, "aria-label": `${t('open')} — ${draft.label}`, onClick: () => { onOpen(draft.id); }, onKeyDown: (event) => { if (event.key === 'Enter')
                onOpen(draft.id); }, children: [(0, jsx_runtime_1.jsx)("span", { className: "dsd-rowIcon", "aria-hidden": "true", children: (0, jsx_runtime_1.jsx)(dsh_client_ui_primitives_1.IconNewChatOutline16, { size: 16 }) }), (0, jsx_runtime_1.jsxs)("span", { className: "dsd-rowBody", children: [(0, jsx_runtime_1.jsx)("span", { className: "dsd-rowTitle", children: t('rowTitle') }), (0, jsx_runtime_1.jsx)("span", { className: "dsd-rowLabel", children: draft.label }), preview !== undefined && (0, jsx_runtime_1.jsx)("span", { className: "dsd-rowPreview", title: preview, children: preview })] }), (0, jsx_runtime_1.jsx)("span", { className: "dsd-rowWhen", children: when }), (0, jsx_runtime_1.jsx)("button", { type: "button", className: "dsd-discard", "aria-label": t('discard'), title: t('discard'), onClick: (event) => {
                        event.stopPropagation();
                        onDiscard(draft.id);
                    }, children: (0, jsx_runtime_1.jsx)(dsh_client_ui_primitives_1.IconCloseOutline16, { size: 14 }) })] }, draft.id));
    });
    return ((0, jsx_runtime_1.jsxs)(jsx_runtime_1.Fragment, { children: [(0, jsx_runtime_1.jsxs)("button", { ref: triggerRef, type: "button", className: `dsd-trigger${wide ? '' : ' dsd-rail'}`, "aria-haspopup": "dialog", "aria-expanded": open, "aria-label": t('triggerLabel', { n: drafts.length }), title: t('triggerLabel', { n: drafts.length }), onClick: () => { setOpen(current => !current); }, children: [(0, jsx_runtime_1.jsx)("span", { className: "dsd-triggerIcon", "aria-hidden": "true", children: (0, jsx_runtime_1.jsx)(dsh_client_ui_primitives_1.IconNewChatOutline16, { size: wide ? 14 : 18 }) }), wide && (0, jsx_runtime_1.jsx)("span", { className: "dsd-triggerLabel", children: t('trigger') }), (0, jsx_runtime_1.jsx)("span", { className: `dsd-count${wide ? '' : ' dsd-countRail'}`, children: drafts.length })] }), open && (0, react_dom_1.createPortal)((0, jsx_runtime_1.jsxs)("div", { ref: panelRef, className: "dsd-panel", role: "dialog", "aria-label": t('header'), style: position === null
                    ? { visibility: 'hidden', left: 0, top: 0 }
                    : { left: position.left, top: position.top }, children: [(0, jsx_runtime_1.jsx)("div", { className: "dsd-panelHeader", children: t('header') }), (0, jsx_runtime_1.jsx)("div", { className: "dsd-rows", children: rows.length > 0 ? rows : (0, jsx_runtime_1.jsx)("div", { className: "dsd-empty", children: t('empty') }) }), (0, jsx_runtime_1.jsx)("div", { className: "dsd-panelFooter", children: (0, jsx_runtime_1.jsxs)("button", { type: "button", className: "dsd-new", onClick: () => {
                                setOpen(false);
                                startSession();
                            }, children: [(0, jsx_runtime_1.jsx)("span", { "aria-hidden": "true", children: (0, jsx_runtime_1.jsx)(dsh_client_ui_primitives_1.IconPlusOutline16, { size: 14 }) }), (0, jsx_runtime_1.jsx)("span", { children: t('newDraft') })] }) })] }), document.body)] }));
}
// ---------------------------------------------------------------------------
// Plugin entry.
// ---------------------------------------------------------------------------
/** Required services (cordis fiber inject). */
exports.inject = ['slots', 'sessions', 'workspaces', 'locale', 'conversation'];
...
function apply(ctx) {
    ...
    ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
        ...
        inject: () => ({
            ...
            discardSession: (sessionId) => {
                void ctx.workspaces.archiveSession(sessionId).catch((error) => {
                    console.warn('[session-drafts] draft discard failed:', error);
                });
            },
            draftPreviewOf: (sessionId) => {
